...
export default function ActualCostSection({
  ...
  const laborGroups = useMemo<LaborGroup[]>(() => {
    ...
    for (const report of reports) {
      ...
      const hours = toSafeNumber(report.hours);
      if (hours <= 0) continue;
    ...
    return Array.from(monthMapByProfile.entries())
      .map(([profileId, byMonth]) => ({
        ...
        entries: Array.from(byMonth.values()).sort((a, b) =>
          a.target_year_month.localeCompare(b.target_year_month)
        ),
      }))
      .sort((a, b) =>
        fullName(
          profileMap.get(a.profile_id)?.last_name,
          profileMap.get(a.profile_id)?.first_name
        ).localeCompare(
          fullName(
            profileMap.get(b.profile_id)?.last_name,
            profileMap.get(b.profile_id)?.first_name
          ),
          "ja"
        )
      );
  ...
  const grandTotal = useMemo(
    () => Array.from(monthTotals.values()).reduce((sum, value) => sum + value, 0),
    [monthTotals]
  );
  ...
  const setMonthEntryAt = (index: number, key: keyof MonthEntry, value: string) => {
    setMonthEntries((current) =>
      current.map((entry, idx) => (idx === index ? { ...entry, [key]: value } : entry))
    );
  };
  ...
  const save = async () => {
    ...
    if (categoryNumber === 1 && !partnerId) {
      return setErrorMsg("パートナーを選択してください。");
    }
    if (categoryNumber === 0 && !expenseName.trim()) {
      return setErrorMsg("費目を入力してください。");
    }

    for (const entry of monthEntries) {
      if (!entry.year || !entry.month) return setErrorMsg("年月を入力してください。");
      if (Number(entry.month) < 1 || Number(entry.month) > 12) {
        return setErrorMsg("月は1〜12で入力してください。");
      }
      const amount = Number(entry.amount);
      if (!Number.isFinite(amount) || amount < 0) {
        return setErrorMsg("金額は0以上の数値で入力してください。");
      }
    }
    ...
    try {
      ...
      setCosts((current) => {
        const filtered = editGroupKey
          ? current.filter((item) => buildManualGroupKey(item) !== editGroupKey)
          : current;

        return [...filtered, ...nextInserted].sort((a, b) =>
          a.target_year_month.localeCompare(b.target_year_month)
        );
      });
          ...
          <thead>
            <tr>
              ...
              {monthLabels.map((label) => (
                <th key={label} className={styles.thMonth}>
                  {toMonthLabel(label)}
                </th>
              ))}
              ...
              allGroups.map((entry) => {
                if (entry.kind === "labor") {
                  ...
                  const ownerLabel = fullName(
                    profileMap.get(group.profile_id)?.last_name,
                    profileMap.get(group.profile_id)?.first_name
                  );
                ...
                const group = entry.group;
                const costMap = new Map(group.entries.map((item) => [item.target_year_month, item]));
                const total = group.entries.reduce((sum, item) => sum + toSafeNumber(item.amount), 0);
                ...
                return (
                  <tr key={group.key}>
                    ...
                    {monthLabels.map((label) => (
                      <td key={label} className={styles.tdMonth}>
                        {costMap.has(label) ? formatYen(toSafeNumber(costMap.get(label)?.amount)) : ""}
                      </td>
                    ))}
                    <td className={styles.tdTotal}>{formatYen(total)}</td>
